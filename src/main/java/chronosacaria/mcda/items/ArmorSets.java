...
public enum ArmorSets implements ArmorMaterial {
    ...
    BATTLE("battle", "battle_robe",
            ...
            () -> ItemID.FABRIC_BOLT_BLACK),
    SPLENDID("splendid", "splendid_robe",
            25,
            ITEM_ARMOR_EQUIP_ELYTRA,
            () -> ItemID.FABRIC_BOLT_PURPLE),
    EVOCATION("evocation", "evocation_robe",
            25,
            ITEM_ARMOR_EQUIP_ELYTRA,
            () -> ItemID.FABRIC_BOLT_BLUE),
    EMBER("ember", "ember_robe",
            25,
            ITEM_ARMOR_EQUIP_ELYTRA,
            () -> ItemID.FABRIC_BOLT_RED),
    VERDANT("verdant", "verdant_robe",
            25,
            ITEM_ARMOR_EQUIP_ELYTRA,
            () -> ItemID.FABRIC_BOLT_GREEN),
    ...
    private final Supplier<ItemConvertible> repairIngredient;
    private Ingredient ingredientCache;

    ArmorSets(String textureName, String setName, int enchantability, 
              SoundEvent soundEvent, Supplier<ItemConvertible> repairIngredient) {
    ...
    public String[] getDefaultRepairIngredient() {
        ItemConvertible c = this.repairIngredient.get();
        if (c instanceof ItemID id) return List.of(Mcda.MOD_ID + ":" + id.toString().toLowerCase(Locale.ROOT)).toArray(String[]::new);
        return List.of(Registry.ITEM.getId(c.asItem()).toString()).toArray(String[]::new);
    }
